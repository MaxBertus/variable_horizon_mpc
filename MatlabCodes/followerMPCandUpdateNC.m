function [p_tp1, X_F, qi, error, u_opt] = followerMPCandUpdateNC(...
                            plant, XL, x0, N, params, obstacles, U_f_old, alg_option)

    [qi, ~] = getObstacleInfo(obstacles, x0(1:2));
    [G,W,S] = rigidBodyConstraints(plant.A, plant.B, x0, [], N, params.u_lim, params.v_lim, params.robotShape);
    ...
    options = optimoptions('fmincon','Algorithm',alg_option,...
        'OptimalityTolerance',1e-2, 'SpecifyObjectiveGradient',true, ...
          "MaxIterations", 5000,"SpecifyConstraintGradient", true,...
        "Display","off");
    
    [u_opt] = fmincon(...
         @(U) followerCostGradientHessian(U,x0,h, pe),...
         U_f_old, G, W + S*x0, [], [], [], [], ...
         @(U) followerNC(U, x0, qi, pe, h), options);
